...
from linevul_model_multi_category import Model
import pandas as pd
# metrics
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        labels = df["cme_category_to_target"].astype(int).tolist()
...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    for batch in tqdm(test_dataloader, desc="evaluate test"):
        ...
        softmax_entry.append({"targets": labels.tolist(),
                              # "feature_list_0": [item.tolist() for item in feature_list[0]],
                              "feature_list_1": [item.tolist() for item in feature_list[1]],
                              "feature_list_2": [item.tolist() for item in feature_list[2]],
    ...
    if len(list(set(y_trues.tolist()))) == 2:
        ...
        import json
        test_type = 'Mix_Project_Resample_Paired_func_after'
        softmax_entry_output = '/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/cross_sections/cross-project/cross_project_result_analysis/softmax_entry'
        args.fold_dir = 'fold_0_dataset'
        softmax_entry_output = os.path.join(softmax_entry_output, 'linevul', args.fold_dir)
        if not os.path.exists(softmax_entry_output):
            os.makedirs(softmax_entry_output)
        with open(os.path.join(softmax_entry_output, test_type + "_softmax_entry.json"), "w") as file:
            json.dump(softmax_entry, file, indent=4)
...
def main():
    ...
    ## parameters
    parser.add_argument("--input_dir", type=str, default='/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/cross_sections/code/data/subsets/multi_category_cross_project/fold_0_holdout')
    parser.add_argument("--train_data_file", default="holdout.csv", type=str, required=False,
                        help="The input training data file (a csv file).")
    parser.add_argument("--output_dir", default="/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/models/LineVul/scripts/multi_category/saved_models/multi_category_cross_project/fold_0_dataset", type=str, required=False,
    ...
    parser.add_argument("--eval_data_file", default="holdout.csv", type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    parser.add_argument("--test_data_file", default="holdout.csv", type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    ...
    if args.do_test:
        ...
        if 'holdout' in args.input_dir:
            args.eval_data_file = 'holdout.csv'
            args.train_data_file = 'holdout.csv'
            args.test_data_file = 'holdout.csv'
