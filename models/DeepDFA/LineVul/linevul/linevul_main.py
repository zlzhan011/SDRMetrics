...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train", return_index=False):
        ...
        func_key = "func_before"  # func_before  func_after
...
def t_test(args, model, tokenizer, test_dataset, flowgnn_dataset, output_dir, best_threshold=0.5):
    ...
    y_preds_trues = pd.DataFrame(
        {"all_inputs_ids": all_inputs_ids, "y_trues": y_trues,
         "DeepDFA_" + args.MSR_processing_flag + "_Predictions": y_preds, "logits": logits[:, 1]})
    # y_preds_trues = pd.DataFrame({"all_inputs_ids": all_inputs_ids, "y_trues": y_trues, "DeepDFA_Predictions": y_preds})
    output_dir_final = os.path.join(args.input_file_dir, 'DeepDFA_LineVul', 'add_after_to_before', args.MSR_processing_flag)
    if not os.path.exists(output_dir_final):
        os.makedirs(output_dir_final)

    y_preds_trues.to_excel(os.path.join(output_dir_final, "test_"+args.input_column+"_prediction.xlsx"))
...
def main():
    ...
    parser.add_argument("--MSR_processing_flag", type=str, default="original", help="dataset name to load for DDFA")
    ...
    # args.MSR_processing_flag = 'original'
    # Setup logging
    ...
    if args.do_test:
        if os.path.exists(args.model_name):
            ...
            #     output_dir = os.path.join(args.output_dir, args.model_name, 'checkpoint-best-f1', 'model.bin')
            output_dir = os.path.join(args.output_dir, 'checkpoint-best-f1', '1234_linevul.bin')
