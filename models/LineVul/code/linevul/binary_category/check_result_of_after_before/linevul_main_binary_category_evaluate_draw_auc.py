...
def re_shuffle_df(df):
    np.random.seed(42)
    df_head_shuffled = df.iloc[:18864].sample(frac=1).reset_index(drop=True)


    df_tail = df.iloc[18864:]
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        func_key = 'func_after'  # func_after  func_before  processed_func
...
def evaluate(args, model, tokenizer, eval_dataset, eval_when_training=False):
    ...
    for batch in tqdm(eval_dataloader, desc="evaluate eval"):
        if args.eval_export:
            (inputs_ids, labels, inputs_index) = [x.to(args.device) for x in batch]
        else:
            (inputs_ids, labels) = [x.to(args.device) for x in batch]
        all_inputs_ids.append(inputs_index)
        with torch.no_grad():
            lm_loss, logit = model(input_ids=inputs_ids, labels=labels)
            loss_one_batch = lm_loss.mean().item()
            eval_loss += lm_loss.mean().item()
            avg_eval_loss = round(eval_loss/nb_eval_steps, 5)
            # print("avg_eval_loss:", avg_eval_loss)
            wandb.log({"test_loss": loss_one_batch, "epoch": 10})
            # wandb.log({"test_loss:", eval_loss, step=nb_eval_steps})
    ...
    draw_auc(args, y_trues, logits)
    if len(list(set(y_trues.tolist()))) in [1, 2]:
        best_threshold = 0.5
        best_f1 = 0
        ...
        if args.evaluate_only_paired_func:
            e = 'evaluate_only_paired_func'
        else:
            e = ''

        y_preds_trues = pd.DataFrame(
            {"all_inputs_ids": all_inputs_ids, "y_trues": y_trues,
             "LineVul_" + args.MSR_processing_flag + "_Predictions": y_preds,
             "logits":logits[:, 1] })
        y_preds_trues.to_excel(
            os.path.join(args.input_file_dir, 'LineVul_Process_func', args.MSR_processing_flag,
                         "test_" + args.input_column+ e + "_prediction.xlsx"))
...
def main():
    ...
    parser.add_argument("--do_test", default=False, action='store_true',
                        help="Whether to run eval on the dev set.")
    parser.add_argument("--eval_export", default=True, action='store_true',
    ...
    if args.do_eval:
        ...
        output_dir = '/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/models/LineVul/code/linevul/binary_category/check_result_of_after_before/MSR_original_dataset_processed_func/visualize_loss_v2/checkpoint-best-f1/model.bin'
