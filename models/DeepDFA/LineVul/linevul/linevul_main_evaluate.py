...
sys.path.append(parent_parent_parent_parent_dir)
print(sys.path)
import random
import re
import json
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train", return_index=False):
        ...
        if args.test_data_file == 'testdataset_before_fixed_and_not_paired.xlsx':
            func_key = 'processed_func'
            target_label = 'target'
        elif args.test_data_file == 'testdataset_after_fixed.xlsx':
            func_key = 'func_after'
            target_label = 'target'
        elif args.test_data_file == 'not_paired.xlsx':
            func_key = 'processed_func'
            target_label = 'target'
        elif args.test_data_file == 'after_before_paired.xlsx':
            func_key = 'func_before'
            target_label = 'target'
            func_key = 'func_after'
            target_label = 'target'

        func_key = 'func_before'
        # if args.paired_filter_flag:
        #     paired_dir = '/data/cs_lzhan011/vulnerability/data-package/models/LineVul/code/data/big-vul_dataset/paired'
        #     paired_file = pd.read_excel(os.path.join(paired_dir, 'testdataset_before_fixed_and_not_paired.xlsx'))
        #     paired_file['files_id'] = paired_file['files_name'].apply(lambda x: int(re.split("_",x)[0]))
        #     paired_file = paired_file['files_id']
        #     # df['files_id'] = df['files_name'].apply(lambda x: int(re.split("_",x)[0]))
        #     df = pd.merge(left=paired_file, right=df, how='left', left_on='files_id', right_on='index')
...
def t_test(args, model, tokenizer, test_dataset, flowgnn_dataset, output_dir, best_threshold=0.5):
    ...
    print("all_inputs_ids:", all_inputs_ids[:10])
    # all_inputs_ids = [item for tensor in all_inputs_ids for item in tensor]
    # all_inputs_ids = np.concatenate(all_inputs_ids_cpu, 0)
    y_preds = logits[:, 1] > best_threshold
    ...
    y_preds_trues = pd.DataFrame({"all_inputs_ids":all_inputs_ids,"y_trues":y_trues,"DeepDFA_Predictions":y_preds})
    y_preds_trues.to_excel(os.path.join(args.input_file_dir, 'DeepDFA',  "train_func_before_prediction.xlsx"))
...
def main():
    ...
    ## parameters
    parser.add_argument("--input_file_dir", type=str, default='/data/cs_lzhan011/vulnerability/data-package/models/LineVul/code/data/big-vul_dataset/paired')
