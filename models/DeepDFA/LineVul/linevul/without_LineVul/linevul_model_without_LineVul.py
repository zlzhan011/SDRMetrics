...
class RobertaClassificationHead(nn.Module):
    ...
    def __init__(self, config, extra_dim):
        ...
        self.dense = nn.Linear(extra_dim, config.hidden_size)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.out_proj = nn.Linear(config.hidden_size, 2)

    def forward(self, flowgnn_embed, features_list_flag, **kwargs):
        ...
        x = self.dense(x)
        x = torch.tanh(x)
        print("x_4 shape:", x.shape)
        x = self.dropout(x)
        print("x_5 shape:", x.shape)
        features_list_flag.append(x)
        x = self.out_proj(x)
        return x, features_list_flag
...
class Model(RobertaForSequenceClassification):   
    ...
    def forward(self, input_embed=None, labels=None, graphs=None, output_attentions=False, input_ids=None, features_list_flag=False):
        ...
        if output_attentions:
            ...
            logits, features_list = self.classifier(flowgnn_embed, features_list)
            print("logits_ shape:", logits.shape)
            features_list.append(logits)
            prob = torch.softmax(logits, dim=-1)
            print("prob_ shape:", prob.shape)
