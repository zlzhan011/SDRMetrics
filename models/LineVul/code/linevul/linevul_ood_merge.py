...
def ood(args, model, tokenizer, eval_dataset, eval_when_training=False, file_type='test'):
    # build dataloader
    eval_sampler = SequentialSampler(eval_dataset)
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, num_workers=0)
    ...
    cache_name = f"cache/multi_"+file_type+"_last_second_layer.npy"
...
def main():
    ...
    ## parameters
    parser.add_argument("--train_data_file", default="train_merge_multi_type.csv", type=str, required=False,
                        help="The input training data file (a csv file).")
    parser.add_argument("--ood_data_file", default="_ttv_ood_type.csv", type=str, required=False,
                        help="The input training data file (a csv file).")
    parser.add_argument("--output_dir", default="./saved_models_two_target_merge", type=str, required=False,
    ...
    parser.add_argument("--eval_data_file", default="valid_merge_multi_type.csv", type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    parser.add_argument("--test_data_file", default="test_merge_multi_type.csv", type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    ...
    parser.add_argument("--do_ood", default=False, action='store_true',
    ...
    if args.do_ood:
        ...
        model.to(args.device)
        test_dataset = TextDataset(tokenizer, args, file_type='eval')
        result = ood(args, model, tokenizer, test_dataset, eval_when_training=True, file_type='valid')

        # exit()
        test_dataset = TextDataset(tokenizer, args, file_type='test')
        result = ood(args, model, tokenizer, test_dataset, eval_when_training=True, file_type='test')

        train_dataset = TextDataset(tokenizer, args, file_type='train')
        result = ood(args, model, tokenizer, train_dataset, eval_when_training=True, file_type='train')

        train_dataset = TextDataset(tokenizer, args, file_type='ood')
        result = ood(args, model, tokenizer, train_dataset, eval_when_training=True, file_type='ood')
