...
def diversevul(cache=True, sample=False):
    """
    Read diversevul dataset from CSV
    """
    savefile = (
        svd.get_dir(svd.cache_dir() / "minimal_datasets")
        / f"minimal_diversevul{'_sample' if sample else ''}.pq"
    )
    if cache:
        try:
            df = pd.read_parquet(savefile, engine="fastparquet").dropna()
            ...
            logger.exception("diversevul exception, loading from source")
    ...
    ].to_csv(svd.cache_dir() / "diversevul/diversevul_metadata.csv", index=0)
    return df
...
def ds_filter(
    ...
    assert len(df) > 0
    df.to_csv(os.path.join(intermediate_result, 'function_name_ds_filter_variable_name_df_after_check_file.csv'))
    ...
    assert len(df) > 0
    df.to_csv(os.path.join(intermediate_result, 'function_name_ds_filter_variable_name_df_after_check_valid.csv'))
