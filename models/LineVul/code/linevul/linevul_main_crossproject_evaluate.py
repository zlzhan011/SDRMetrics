...
def visual(args, model, tokenizer, eval_dataset, eval_when_training=False, file_type='test', calculate_metric=False):
    ...
    cache_name = f"cache_visual/multi_" + file_type + "_last_second_layer.npy"
...
def main():
    ...
    if args.do_ood:
        ...
        fold_times = 5
        fold_holdout_dir = "fold_"+str(fold_times) +"_holdout"
        fold_model_dir = "fold_"+str(fold_times)+"_dataset"
        args.output_dir = '/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/models/LineVul/scripts/saved_models/cross_project/' + fold_model_dir
        output_dir = os.path.join(args.output_dir, 'checkpoint-best-f1', 'model.bin')
        model.load_state_dict(torch.load(output_dir, map_location=args.device))
        model.to(args.device)
        args.holdout_dir = '/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/cross_sections/code/data/subsets/cross_project/' + fold_holdout_dir
        # args.holdout_dir = '/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/cross_sections/code/data/subsets/cross_project/' + fold_model_dir

        for file in os.listdir(args.holdout_dir):
            if "holdout.csv" in file:
                print("file", file)
