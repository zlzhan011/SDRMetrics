...
def obtain_A_B_N(args, option, layer_number, target_file, input_dir):
    ...
    for file in target_file:
        ...
        if option == 1:
            ...
            elif 'setting_6_add_after' in file:
                feature, correct_log, label_log = load_similar_func_feature(cache_name, step_state=layer_number,
                                                                            return_label=True)
                after_feature = []
                after_label = []
                after_section = len(label_log) - 906
                for ii in range(len(label_log)):
                    if ii <= after_section:
                        continue
                    i_label = label_log[ii]
...
def visualize_llc_v2(llc_sum, args):
    ...
    def custom_scale(y):
        # return np.where(y < 0.065, y * 50, y + 0.22)
        return np.where(y < 0.065, y * 50, np.where(y <= 0.50, y * 10 + 0.2, y ))
    ...
    custom_ticks = np.concatenate([np.linspace(5.4, 8.7, 5), np.linspace(0.033, 0.062, 3), np.linspace(0.344, 0.468, 3)])
    custom_tick_labels = [f'{tick:.3f}' for tick in np.linspace(5.4, 8.7, 5)] + [f'{tick:.3f}' for tick in
                                                                                    np.linspace(0.033,0.062, 3)] + [
                             f'{tick:.3f}' for tick in
                             np.linspace(0.344, 0.468, 3)]
...
if __name__ == '__main__':
    ...
    llc_sum = pd.read_csv("llc_sum.csv")
    visualize_llc_v2(llc_sum, args)
