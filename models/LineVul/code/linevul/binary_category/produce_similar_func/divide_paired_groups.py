...
def calcul_levenshtein_distance(df):
    ...
    CVE_ID = df[df['target'] == 1]['CVE ID'].tolist()
    CWE_ID = df[df['target'] == 1]['CWE ID'].tolist()

    if  args.using_column_index :
        indices_v = df[df['target'] == 1]['index'].tolist()
        indices_nv = df[df['target'] == 0]['index'].tolist()
    else:
        indices_v = df[df['target'] == 1].index.tolist()
        indices_nv = df[df['target'] == 0].index.tolist()
    ...
    for i in tqdm(range(len(indices_v)), desc="load dataset"):
        ...
        intermediate_results = {"file_id":index,
                                "one_CVE_ID": one_CVE_ID,
                                "one_CWE_ID": one_CWE_ID,
