...
from models.LineVul.code.linevul.binary_category.check_result_of_after_before.linevul_model_binary_category import Model
import pandas as pd
# metrics
    ...
    def __getitem__(self, i):
        if self.return_index:
            return torch.tensor(self.examples[i].input_ids), torch.tensor(self.examples[i].label), torch.tensor(
                self.examples[i].index), torch.tensor(
...
def my_collate_fn(batch):
    ...
    # 填充输入序列
    input_ids_padded = pad_sequence([torch.tensor(ids) for ids in input_ids_list],
                                    batch_first=True, padding_value=1)  # 假设0是填充值
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        if file_path.endswith(".jsonl"):
            ...
            index_file_name = df.set_index('index')['file_name'].to_dict()
            args.index_file_name = index_file_name
        ...
        func_key = 'function_content'  # func_after  func_before
...
def evaluate(args, model, tokenizer, eval_dataset, eval_when_training=False):
    ...
    dummy_input = torch.zeros((1, 512)).cuda()
    dummy_input = torch.tensor(dummy_input).to(torch.int64)
    # print("dummy_input_--shape:", dummy_input.shape)
    score, feature_list = model(input_ids=dummy_input, output_feature_list=True)
    if len(feature_list[0].shape) == 3:
        feature_list[0] = feature_list[0].mean(dim=1)
    ...
    # cache_name = os.path.join(args.produce_similar_func_dir, str(args.dir_index), "produce_similar_" + str(args.dir_index) + "_last_second_layer.npy")
    if args.chen_task:
        cache_name = os.path.join(args.input_dir, 'embedding_output',args.MSR_processing_flag)
    ...
    for batch in eval_dataloader:
        ...
        cache_name_one_file = os.path.join(cache_name, args.index_file_name[batch_idx])
        print("cache_name_one_file----:", cache_name_one_file)
        ...
        np.savez(cache_name_one_file, feat_log.T, score_log.T, label_log, predict_log)
        # np.savez(cache_name_one_file, out.T, score.T, labels, y_preds)
    #calculate scores
    ...
    if len(list(set(y_trues.tolist())))==2 or len(list(set(y_trues.tolist())))==1:
        ...
        #     {"all_inputs_ids": all_inputs_ids, "y_trues": y_trues, "LineVul_"+args.MSR_processing_flag+"_Predictions": y_preds})
        # y_preds_trues.to_excel(
        #     os.path.join(args.input_file_dir, 'LineVul', args.MSR_processing_flag, "test_" + args.input_column+"_prediction.xlsx"))
...
def main():
    ...
    if args.visualize_loss:
        args.output_dir = os.path.join(args.output_dir, 'visualize_loss')
        wandb.login(key="47f89ceb3eeec066f111150006e02c5ebcfb52f3")
        wandb.init(project='Vulnerable', name=args.MSR_processing_flag + "_produce_similar_func")
        wandb.config = {
