...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        if "processed_func" in df.columns:
            ...
            func_key = "func"
        func_key = "func_after"  # func_after  func_before
        label_name = "target"
        args.func_key = func_key
...
def visual(args, model, tokenizer, eval_dataset, eval_when_training=False, file_type='test', calculate_metric=False):
    ...
    file_id_log = np.zeros(len(eval_dataloader.dataset))
    ...
    cache_name = os.path.join(args.output_file_dir, args.test_data_file + args.func_key + "_last_second_layer_index_correct.npy")
    batch_size = eval_dataloader.batch_size
    batch_idx = 0
    for batch in tqdm(eval_dataloader, desc="evaluate eval"):
        (inputs_ids, labels, file_id) = [x.to(args.device) for x in batch]
        with torch.no_grad():
            start_ind = batch_idx * batch_size
            ...
            file_id_log[start_ind:end_ind] = file_id.cpu()
            # if batch_idx % 100 == 0:
            #     print(f"{batch_idx}/{len(out_loader)}")

        nb_eval_steps += 1
        batch_idx += 1
    np.savez(cache_name, feat_log.T, score_log.T, label_log, predict_log, file_id_log)
    result = ""
...
def main():
    ...
    if args.do_visual:
        ...
        model.to(args.device)
        input_parent_dir = '/data/cs_lzhan011/vulnerability/data-package/models/LineVul/code/data/big-vul_dataset/generate_non_paired_random_pair/MSR'
        input_parent_dir = '/data/cs_lzhan011/vulnerability/data-package/models/LineVul/code/data/Diversevul/ttv/generate_non_paired_random_pair/Diversevul'
        visual_dir = os.path.join(input_parent_dir,'visual')
        args.input_file_dir = input_parent_dir
        ...
        for file in [0]:
            ...
            if calculate_metric:
                args.test_data_file = 'paired_df_distance_0_index_correct.csv'  # df_nv_all_random_pair.csv    df_v_all_paired.csv  df_v_all_paired_index_correct.csv  df_nv_all_random_pair_index_correct.csv
            paired_flag=True
            calculate_metric = True
            if paired_flag:
                args.test_data_file = 'paired_df_distance_0_index_correct.csv'  # df_nv_all_random_pair.csv  df_v_all_paired.csv
            ...
            result = visual(args, model, tokenizer, test_dataset, eval_when_training=True, file_type='test', calculate_metric=calculate_metric)
            exit()
