...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        if "processed_func" in df.columns:
            ...
            func_key = "func"
        func_key = "func_after"
        label_name = "targets_after"
        print(list(df.columns))
        funcs = df[func_key].tolist()
        labels = df[label_name].astype(int).tolist()
...
def visual(args, model, tokenizer, eval_dataset, eval_when_training=False, file_type='test', calculate_metric=False):
    # build dataloader
    eval_sampler = SequentialSampler(eval_dataset)
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, num_workers=0)

    dummy_input = torch.zeros((16, 512)).cuda()
    dummy_input = torch.tensor(dummy_input).to(torch.int64)
    # print("dummy_input_--shape:", dummy_input.shape)
    score, logit, feature_list = model.feature_list(input_ids=dummy_input)
    ...
    logits = []
    y_trues = []
    feat_log = np.zeros((len(eval_dataloader.dataset), sum(featdims)))
    score_log = np.zeros((len(eval_dataloader.dataset), num_classes))
    label_log = np.zeros(len(eval_dataloader.dataset))
    predict_log = np.zeros(len(eval_dataloader.dataset))
    ...
    #     print("inputs___shape:", inputs.shape)
    ...
    cache_name = os.path.join(args.output_file_dir, "multi_" + file_type + "_last_second_layer.npy")
    ...
    for batch in tqdm(eval_dataloader, desc="evaluate eval"):
        ...
        with torch.no_grad():
            start_ind = batch_idx * batch_size
            end_ind = min((batch_idx + 1) * batch_size, len(eval_dataloader.dataset))
            # print("inputs___shape:", inputs_ids.shape)
            # lm_loss, logit = model(input_ids=inputs_ids, labels=labels)
            # lm_loss, logit, para = model.feature_list(input_ids=inputs_ids, labels=labels)
            score, logit, feature_list = model.feature_list(input_ids=inputs_ids)
            # eval_loss += lm_loss.mean().item()
            ...
            y_preds = logits_np[start_ind:end_ind, 1] > best_threshold

            # out = torch.cat([F.adaptive_avg_pool2d(layer_feat, (layer_feat.shape[1],1)).squeeze() for layer_feat in feature_list], dim=1)
            out = torch.cat([layer_feat for layer_feat in feature_list], dim=1)

            feat_log[start_ind:end_ind, :] = out.data.cpu().numpy()
            label_log[start_ind:end_ind] = labels.data.cpu().numpy()
            score_log[start_ind:end_ind] = score.data.cpu().numpy()
            predict_log[start_ind:end_ind] = y_preds
            # if batch_idx % 100 == 0:
            #     print(f"{batch_idx}/{len(out_loader)}")

        nb_eval_steps += 1
        batch_idx += 1
    np.savez(cache_name, feat_log.T, score_log.T, label_log, predict_log)
    result = ""
    ...
    y_preds_trues = pd.DataFrame({"y_trues":y_trues,"LineVul_Predictions":y_preds})
    y_preds_trues.to_excel(os.path.join(args.input_file_dir, "after_before_paired_func_after_prediction.xlsx"))
...
def write_raw_preds_csv(args, y_preds):
    ...
    df.to_csv("./results/synthetic_testv3.csv", index=False)
...
def main():
    ...
    ## parameters
    parser.add_argument("--ood_data_file", default="after_before_paired.xlsx", type=str, required=False,
                        help="The input training data file (a csv file).")
    parser.add_argument("--train_data_file", default="synthetic_testv3.csv", type=str, required=False,
                        help="The input training data file (a csv file).")
    # parser.add_argument("--output_dir", default="/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/models/LineVul/code/linevul/saved_models_vulnerable", type=str, required=False,
    #                     help="The output directory where the model predictions and checkpoints will be written.")
    parser.add_argument("--output_dir",
                        default="/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/models/LineVul/code/linevul/saved_models_reveal_split_dataset",
    ...
    parser.add_argument("--eval_data_file", default="after_before_paired.xlsx", type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    parser.add_argument("--test_data_file", default="after_before_paired.xlsx", type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    ...
    parser.add_argument("--do_ood", default=True, action='store_true',
                        help="Whether to run ood.")
    parser.add_argument("--do_visual", default=True, action='store_true',
                        help="Whether to run ood.")
    parser.add_argument("--do_visual_cross_project", default=False, action='store_true',
                        help="Whether to run ood.")
    ...
    # # do_visual_cross_project
    # if args.do_visual_cross_project:
    #     print("args.output_dir:", args.output_dir)
    #     print("args.model_name:", args.model_name)
    #
    #     input_parent_dir = '/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/cross_sections/code/data/subsets/cross_project'
    #     output_dir = '/data/cs_lzhan011/vulnerability/data-package/models/LineVul/code/data/big-vul_dataset'
    #
    #     for k_fold in range(6):
    #         fold_model_dir = "fold_" + str(k_fold) + "_dataset"
    #         args.fold_model_dir = '/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/models/LineVul/scripts/saved_models/cross_project/' + fold_model_dir
    #         args.fold_model_dir = os.path.join(args.fold_model_dir, 'checkpoint-best-f1', 'model.bin')
    #         model.load_state_dict(torch.load(args.fold_model_dir, map_location=args.device))
    #         model.to(args.device)
    #
    #         for dir_type in ['fold_' + str(k_fold) + '_dataset', 'fold_'+str(k_fold)+'_holdout']:
    #             visual_dir = os.path.join(input_parent_dir, dir_type)
    #             args.input_file_dir = visual_dir
    #             args.output_file_dir = os.path.join(output_dir, 'visual_cache')
    #             # args.input_parent_dir = input_parent_dir
    #             for file in os.listdir(visual_dir):
    #                 # if 'test.csv' in file or 'holdout.csv' in file:
    #                 if 'fold_' in file:
    #                     args.test_data_file = file
    #                     calculate_metric = False
    #                     if calculate_metric:
    #                         args.test_data_file = 'synthetic_testv3.csv'
    #                     test_dataset = TextDataset(tokenizer, args, file_type='test')
    #                     result = visual(args, model, tokenizer, test_dataset, eval_when_training=True, file_type=dir_type+"_"+file[:-4],
    #                                     calculate_metric=calculate_metric)
    #     exit()
    #     # train_dataset = TextDataset(tokenizer, args, file_type='train')
    #     # result = visual(args, model, tokenizer, train_dataset, eval_when_training=True,
    #     #              file_type='value_error_devign_train')
    #     #
    #     # train_dataset = TextDataset(tokenizer, args, file_type='ood')
    #     # result = visual(args, model, tokenizer, train_dataset, eval_when_training=True, file_type='value_error_devign_ood')


    print("finish do_visual_cross_project")
    # exit()
    print("do_visual --------\n\n")
    # visual
    results = {}
    if args.do_visual:
        ...
        model.to(args.device)
        input_parent_dir = '/data/cs_lzhan011/vulnerability/data-package/models/LineVul/code/data/big-vul_dataset'
        visual_dir = os.path.join(input_parent_dir,'visual')
        args.input_file_dir = visual_dir
        args.output_file_dir = os.path.join(input_parent_dir,'visual_cache')
        args.input_parent_dir = input_parent_dir
        for file in os.listdir(visual_dir):
            args.test_data_file = file
            calculate_metric = False
            if calculate_metric:
                args.test_data_file = 'after_before_paired.xlsx'
            paired_flag=True
            ...
            test_dataset = TextDataset(tokenizer, args, file_type='test')
            result = visual(args, model, tokenizer, test_dataset, eval_when_training=True, file_type=file[:-4], calculate_metric=calculate_metric)
            exit()
        # train_dataset = TextDataset(tokenizer, args, file_type='train')
        # result = visual(args, model, tokenizer, train_dataset, eval_when_training=True,
        #              file_type='value_error_devign_train')
    ...
    if args.eval_export:
        ...
        result_df.to_csv(os.path.join(os.path.dirname(args.train_data_file),
