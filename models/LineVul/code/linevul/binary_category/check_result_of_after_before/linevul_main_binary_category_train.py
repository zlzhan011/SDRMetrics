...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        if args.MSR_processing_flag == 'add_after_to_before':
            ...
            if args.MSR_processing_flag in ['diversevul_before', 'diversevul_before_down_resample',
                                            'diversevul_down_resample_add_after']:
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    args.batch_loss_dir = batch_loss_dir
    global_step = 0
    ...
    for idx in range(args.epochs):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    if results['eval_f1'] > best_f1:
                        ...
                        torch.save(model_to_save.state_dict(), output_dir+"_" + str(best_f1))
        ...
        # if not os.path.exists(batch_loss_dir):
        #     os.makedirs(batch_loss_dir)
        loss_file = os.path.join(batch_loss_dir, args.MSR_processing_flag + 'batch_loss_' + str(idx_10) + '.csv')
...
def evaluate(args, model, tokenizer, eval_dataset, eval_when_training=False):
    ...
    all_test_loss = pd.DataFrame({"all_test_loss":all_test_loss})
    if not args.do_train and args.do_eval:
        args.epoch_index = 9
        args.batch_loss_dir = os.path.join(args.output_dir, 'batch_loss_training_dataset')
        if not os.path.exists(args.batch_loss_dir):
            os.makedirs(args.batch_loss_dir)

    loss_file = os.path.join(args.batch_loss_dir, args.MSR_processing_flag + 'batch_loss_' + str(args.epoch_index) + '_test.csv')


    if args.model_shuffle_test_append:
        loss_file = os.path.join(args.batch_loss_dir,
                                 args.MSR_processing_flag + 'batch_loss_' + str(args.epoch_index) + '_test_model_shuffle_test_append.csv')

    print("loss_file:", loss_file)
    all_test_loss.to_csv(loss_file)
    # calculate scores
    ...
    if len(list(set(y_trues.tolist()))) == 2:
        ...
        y_preds_trues = pd.DataFrame(
            {"all_inputs_ids": all_inputs_ids, "y_trues": y_trues,
             "LineVul_" + args.MSR_processing_flag + "_Predictions": y_preds, 'logits': logits[:, 1]})
...
def main():
    ...
    parser.add_argument("--input_dir", type=str,
                        default='/data/cs_lzhan011/vulnerability/data-package/models/LineVul/code/data/big-vul_dataset')
    ...
    # MSR_processing_flag = 'original_save_loss'
    # MSR_processing_flag = 'add_after_into_before_append'
    # MSR_processing_flag = 'add_after_into_before_shuffle'
    ...
    if MSR_processing_flag == 'original':
        ...
        if not os.path.exists(args.output_dir):
            os.mkdir(args.output_dir)
    elif MSR_processing_flag == 'original_save_loss':
        args.input_dir = args.input_dir
        args.output_dir = os.path.join(args.output_dir, MSR_processing_flag)
        args.func_key = 'func_before'  # 'func_before'  # func_after  func_before processed_func  processed_func
        args.label_column = 'target'
        # args.train_data_file = 'test.csv'
    elif MSR_processing_flag == 'add_after_into_before_append':
        args.input_dir = '/data2/cs_lzhan011/vulnerability/dataset/MSR/add_after_into_before/add_after_into_before_append'
        args.output_dir = os.path.join(args.output_dir, MSR_processing_flag)
        args.func_key = 'func_before'  # 'func_before'  # func_after  func_before processed_func  processed_func
        args.label_column = 'target'
        # args.train_data_file = 'test.csv'
    elif MSR_processing_flag == 'add_after_into_before_shuffle':
        args.input_dir = '/data2/cs_lzhan011/vulnerability/dataset/MSR/add_after_into_before/add_after_into_before_shuffle'
        args.output_dir = os.path.join(args.output_dir, MSR_processing_flag)
        args.func_key = 'func_before'  # 'func_before'  # func_after  func_before processed_func  processed_func
        args.label_column = 'target'
        # args.train_data_file = 'test.csv'
        args.model_shuffle_test_append = True
    else:
    ...
    if args.do_eval:
        ...
        if args.MSR_processing_flag == 'add_after_into_before_shuffle' and args.model_shuffle_test_append:
            args.input_dir  = '/data2/cs_lzhan011/vulnerability/dataset/MSR/add_after_into_before/add_after_into_before_append'
        eval_dataset = TextDataset(tokenizer, args, file_type='eval')
        result = evaluate(args, model, tokenizer, eval_dataset)
        if args.do_visual:
            args.output_file_dir = args.output_dir
            visual(args, model, tokenizer, eval_dataset, eval_when_training=False, file_type='test', calculate_metric=True)
    if args.do_test:
