...
from not_use_joern_error import not_use_error_graph
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train", return_index=False):
        ...
        if file_path.endswith(".jsonl"):
            ...
            df_no_after = file_path.replace("_add_after", "")
            df_no_after = pd.read_csv(df_no_after)
            df_no_after_len = df_no_after.shape[0]
            df_no_after = df_no_after.sample(frac=1, random_state=42).reset_index(drop=True)
            if "_add_after" in file_path:
                df = pd.read_csv(file_path)
                # df_before = df.head(df_no_after_len)
                df_after = df.iloc[df_no_after_len:]
                df_after = df_after.sample(frac=1, random_state=42).reset_index(drop=True)
                df = pd.concat([df_no_after, df_after])
            else:
                df = df_no_after
            ...
            if 'index' in df.columns.tolist():
                ...
                df = pd.read_csv(file_path, index_col=0)
            if args.filter_joern_error:
                df = not_use_error_graph(df)
                print("\n\n\ndf shape -------:", df.shape)
        else:
            raise NotImplementedError(file_path)
...
def visual(args, model, tokenizer, test_dataset, flowgnn_dataset, output_dir, best_threshold=0.5):
    ...
    cache_name = os.path.join(os.path.dirname(output_dir), "DeepDFA_on_MSR" + "_last_second_layer_target_0.npy")
