...
class BaseModule(pl.LightningModule):
    ...
    def training_step(self, batch_data, batch_idx):
        intermediate_result_batch_idx = os.path.join(intermediate_result, str(batch_idx))
        if os.path.exists(intermediate_result_batch_idx):
            shutil.rmtree(intermediate_result_batch_idx)
            os.mkdir(intermediate_result_batch_idx)
        else:
            os.mkdir(intermediate_result_batch_idx)

        batch, extrafeats = batch_data
        print("\n\n\n\n********************************\nbatch:", batch)
        with open(os.path.join(intermediate_result_batch_idx, 'function_name_training_step_variable_name_extrafeats.pkl'), 'wb') as f:
            pickle.dump(extrafeats, f)
        with open(os.path.join(intermediate_result_batch_idx, 'function_name_training_step_variable_name_batch.pkl'), 'wb') as f:
            pickle.dump(batch, f)

        label = self.get_label(batch)
        print("label shape:", label.shape)
        with open(os.path.join(intermediate_result_batch_idx, 'function_name_training_step_variable_name_label.pkl'), 'wb') as f:
            pickle.dump(label, f)
        out = self.forward(batch, extrafeats)
        print("out 11 shape:", out.shape)
        with open(os.path.join(intermediate_result_batch_idx, 'function_name_training_step_variable_name_out.pkl'), 'wb') as f:
            pickle.dump(label, f)
        if self.hparams.label_style == "dataflow_solution_in":
            print("self.hparams.label_style == dataflow_solution_in")
        ...
        if (
            ...
            print("self.hparams.undersample_node_on_loss_factor is not None")
