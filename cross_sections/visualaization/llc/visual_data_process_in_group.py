...
def calculate_llc():
    ...
    if option == 1:
        ...
        input_dir = '/data/cs_lzhan011/vulnerability/data-package/models/LineVul/code/data/big-vul_dataset/visual_cache/'
        args.input_dir = input_dir
        target_file = ['test.csvfunc_before_last_second_layer_index_correct.npy.npz', 'test.csvfunc_after_last_second_layer_index_correct.npy.npz']
...
def visualize_llc_v2(llc_sum, args):
    ...
    custom_ticks = np.concatenate([np.linspace(0.01, 0.022, 7), np.linspace(0.1, 0.2, 5), np.linspace(0.2, 0.7, 5)])
    custom_tick_labels = [f'{tick:.3f}' for tick in np.linspace(0.01, 0.022, 7)] + [f'{tick:.3f}' for tick in
                                                                                    np.linspace(0.1, 0.2, 5)] + [
                             f'{tick:.1f}' for tick in
